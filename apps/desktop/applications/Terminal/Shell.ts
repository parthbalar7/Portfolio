...
export const HomeDirectory = '/Users/parth/'
...
export class Shell {
  ...
  private buildRelativePath(path: string): string {
    ...
    const isHomeDirectory = pathItems.length >= 2 && pathItems[0] === "Users" && pathItems[1] === "parth";
  ...
  public process(command: string): void {
    ...
    function handleCommand(applicationName: string, shell: Shell, args: string[], apis: SystemAPIs): void {
      ...
      shell.getTerminal().writeResponse(`psh: command not found: ${applicationName}`);
    }
        ...
        case 'ps': {
          ...
          if (title.length === 0) {
            this.terminal.writeResponseLines([
              'psh: ps requires a value to be set',
