...
export function createBaseFileSystem(): FileSystem {
  ...
  const parth = fileSystem.addDirectory(users, 'parth', false, true);

  const desktop = fileSystem.addDirectory(parth, 'Desktop', false, true);
  const documents = fileSystem.addDirectory(parth, 'Documents', false, true, documentsFolderIcon);
  const trashCanIcon = { src: '/icons/trash-icon.png', alt: 'Trash can icon' };
  const trash = fileSystem.addDirectory(parth, 'Trash', false, true, trashCanIcon);
...
function entriesWithinSelection(entries: Point[], x: number, y: number, dimensions: { width: number, height: number }): number {
  ...
  // NOTE: The width & height are used for both the new incoming entry as the already existing entries.
